...
def aggregate_files(
    ...
    if to_decibels:
        ...
        tfr_data = (
            (tfr_data.loc[:, cols].sub(baseline, axis=0)).div(baseline, axis=0)
        ) * 100
    ...
    with h5py.File(filename, mode="w") as store:
        for (hemi, cluster, freq), data in agg.groupby(["hemi", "cluster", "freq"]):
            ...
            for index in data.index.names:
                ...
                dset.attrs["rows_" + index] = index_vals
...
def hdf2agg(filenames, hemi=None, cluster=None, freq=None):
    return pd.concat([_hdf2agg(f, hemi, cluster, freq) for f in ensure_iter(filenames)])
        ...
        for fhemi, hd in store.items():
            ...
            for fcluster, cd in hd.items():
                if (cluster is not None) and (
                    not any([str(c) == fcluster for c in ensure_iter(cluster)])
                ):
                    continue
...
def aggregate(tfr_data, hemis, all_clusters=None):
    ...
    for hemi, cluster in product(hemis, all_clusters.keys()):
        ...
        tfrs_rh = [
            t for t in tfr_areas if any([a.lower() in t.lower() for a in tfrs_rh])
        ]
        tfrs_lh = [
            t for t in tfr_areas if any([a.lower() in t.lower() for a in tfrs_lh])
        ]
        ...
        if (len(left) == 0) or (len(right) == 0):
            print(
                "%s: Left or right Hemi is empty, skipping for aggregation"
                % (cluster)
            )
            continue
